...
export default function AptitudeTest() {
  ...
  useEffect(() => {
    async function fetchQuestions() {
      try {
        const res = await apiClient.get("/rounds/aptitude");
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      // Prepare answers array ordered by question index
      const ansArray = questions.map((_, idx) => answers[idx] || "");
      const res = await apiClient.post("/rounds/aptitude/submit", { answers: ansArray });
      setResult(res.data);
      if (res.data.passed) {
        alert("You passed! Proceeding to Technical round.");
        navigate("/technical");
      }
    } catch {
  ...
  return (
    <section>
      <h2>Aptitude / Logical Reasoning Test</h2>
